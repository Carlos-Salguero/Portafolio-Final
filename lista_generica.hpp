...
int Lista<TIPODATO>::tamano(){
	cout << "Obteniendo tamano de la lista..." << endl;
	return this->cuenta;
}
...
void Lista<TIPODATO>::insertar(int indice, TIPODATO item){
	...
	//desplaza elemento a partir del indice derecho
	...
	cout << " Inserte elemento " << item << " en el indice " << indice << endl;
	this->items[indice]=item;
	
	//incrementar laa cuenta de elementos
	cout << " Incrementa la cuenta de elementos" << endl;
	this->cuenta++;
}
...
//valor del elemento en el indice x
template <typename TIPODATO>
...
TIPODATO Lista<TIPODATO>::remover(int indice){
	//validar indice
	if  (this->estaVacia()) {
		throw "No se puede remover elementos de una lista vacia";
	}
	if (indice<0 || indice>=this->cuenta) {
		throw "Indice fuera de rango";
	}
	
	cout << "Remover el elemento con indice" << indice << "..." << endl;
	...
	cout<< " Reduciendo l cuenta de elementos" << endl;
	this->cuenta--;
	return valor;
}
